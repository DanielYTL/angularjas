...
(function() {
    ...
    angular
        .module('app')
        .controller('AppControllerBooking', AppControllerBooking);

    AppControllerBooking.$inject = ['$scope', '$filter', 'Booking', 'messageBag'];

    function AppControllerBooking($scope, $filter, Contact, messageBag) {
        //Assign Contact service to $scope variable
        $scope.contacts = Contact;
        //Assign Messages service to $scope variable
        $scope.messages = messageBag;
        ...
        var getHeadings = function(contacts) {
            ...
            for(var i = 0; i<contacts.length; i++) {
                //Get the first letter of a contact's firstName
                var startsWithLetter = contacts[i].bookingDate.toUpperCase();
                //If we have encountered that first letter before then add the contact to that list, else create it
                if(headings.hasOwnProperty(startsWithLetter)) {
                    headings[startsWithLetter].push(contacts[i]);
                } else {
...
(function() {
    ...
    angular
        .module('app')
        .controller('AppControllerHotel', AppControllerHotel);

    AppControllerHotel.$inject = ['$scope', '$filter', 'Hotel', 'messageBag'];

    function AppControllerHotel($scope, $filter, Contact, messageBag) {
        //Assign Contact service to $scope variable
        $scope.contacts = Contact;
        //Assign Messages service to $scope variable
        $scope.messages = messageBag;
        ...
        var getHeadings = function(contacts) {
            ...
            for(var i = 0; i<contacts.length; i++) {
                //Get the first letter of a contact's firstName
                var startsWithLetter = contacts[i].hotelName.charAt(0).toUpperCase();
                //If we have encountered that first letter before then add the contact to that list, else create it
                if(headings.hasOwnProperty(startsWithLetter)) {
                    headings[startsWithLetter].push(contacts[i]);
                } else {
