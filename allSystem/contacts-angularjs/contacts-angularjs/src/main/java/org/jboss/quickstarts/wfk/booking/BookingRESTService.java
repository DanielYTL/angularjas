...
public class BookingRESTService {
    ...
    public Response retrieveBookingByCustomer(@PathParam("id") long id) {
        List<Booking> bookings = service.findByCustomerId(id);
        if (bookings == null) {
            throw new WebApplicationException(Response.Status.NOT_FOUND);
        }
    ...
    public Response retrieveBookingByHotel(@PathParam("id") long id) {
        List<Booking> bookings = service.findByHotelId(id);
        if (bookings == null) {
            throw new WebApplicationException(Response.Status.NOT_FOUND);
        }
    ...
    @GET
    @Path("/taxis/{id:[0-9]+}")
    public Response retrieveBookingByTaxi(@PathParam("id") long id) {
        Booking booking = service.findAllByTaxi(id);
        if (booking == null) {
            throw new WebApplicationException(Response.Status.NOT_FOUND);
        }   
    ...
    public Response retrieveBookingByFlight(@PathParam("id") long id) {
        Booking booking = service.findAllByFlight(id);
        if (booking == null) {
            throw new WebApplicationException(Response.Status.NOT_FOUND);
        }
    ...
    public Response createBooking(Booking booking) {
        log.info("createBooking started. Booking = " + booking.getId() + " " + booking.getCustomer()+ " " + booking.getHotel() + " " 
    +booking.getTaxiid()+ " " +booking.getFlightID() +" "+ booking.getBookingDate());
        if (booking == null) {
        ...
        try {
            ...
            log.info("createBooking completed. Booking = " + booking.getId() + " " + booking.getCustomer()+ " " + booking.getHotel() + " " 
            + booking.getTaxiid()+ " " +booking.getFlightID() +" "+ booking.getBookingDate());
            ...
            if(e.toString().contains("booking"))
                responseObj.put("booking", "That hotel is already booked that day");
                builder = Response.status(Response.Status.CONFLICT).entity(responseObj);
        } catch (Exception e) {
    ...
    public Response updateBooking(@PathParam("id") long id, Booking booking) {
        ...
        log.info("updateBooking started. Booking = " +  booking.getId() + " " + booking.getCustomer()+ " " + booking.getHotel() + " " 
        + booking.getTaxiid()+ " " +booking.getFlightID() +" "+ booking.getBookingDate());
        ...
        try {
            ...
            log.info("updateBooking completed. Booking = " + booking.getId()+ " " + booking.getCustomer()+ " " + booking.getHotel() + " " 
            + booking.getTaxiid()+ " " +booking.getFlightID() +" "+ booking.getBookingDate());
        } catch (ConstraintViolationException ce) {
            log.info("ConstraintViolationException - " + ce.toString());
            // Handle bean validation issues
