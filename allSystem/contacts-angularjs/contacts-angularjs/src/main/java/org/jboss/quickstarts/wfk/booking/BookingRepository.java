...
public class BookingRepository {
    ...
    Booking findByFlight(Long flightID) {
        TypedQuery<Booking> query = em.createNamedQuery(Booking.FIND_BY_FLIGHT, Booking.class).setParameter("flight_ID", flightID); 
        return query.getSingleResult();
    }
	List<Booking> findListByFlight(Long flightID) {
		TypedQuery<Booking> query = em.createNamedQuery(
				Booking.FIND_BY_FLIGHT, Booking.class).setParameter("flight_ID", flightID);
		return query.getResultList();
	}
	Booking findByTaxi(Long taxiid) {
        TypedQuery<Booking> query = em.createNamedQuery(Booking.FIND_BY_TAXI, Booking.class).setParameter("taxi_id", taxiid); 
        return query.getSingleResult();
    }
	List<Booking> findListByTaxi(Long taxiid) {
		TypedQuery<Booking> query = em.createNamedQuery(
				Booking.FIND_BY_TAXI, Booking.class).setParameter("taxi_id", taxiid);
		return query.getResultList();
	}
