...
public class BookingRepository {
    ...
    Booking findByBookingDate(Date bookingDate) {
        TypedQuery<Booking> query = em.createNamedQuery(Booking.FIND_BY_BOOKINGDATE, Booking.class).setParameter("booking_Date", bookingDate); 
    ...
    Booking findByCustomerID(Long customerID) {
        TypedQuery<Booking> query = em.createNamedQuery(Booking.FIND_BY_CUSTOMERID, Booking.class).setParameter("customer_ID", customerID); 
        return query.getSingleResult();
    }
    Booking findByFlightID(Long flightID) {
        TypedQuery<Booking> query = em.createNamedQuery(Booking.FIND_BY_FLIGHTID, Booking.class).setParameter("flight_ID", flightID); 
        return query.getSingleResult();
    }
	List<Booking> findListByFlightID(Long flightID) {
		TypedQuery<Booking> query = em.createNamedQuery(
				Booking.FIND_BY_FLIGHTID, Booking.class).setParameter("flight_ID", flightID);
		return query.getResultList();
	}
    
    /**
     * <p>Returns a single Contact object, specified by a String bookingDate.<p/>
     *
     * <p>If there is more then one, only the first will be returned.<p/>
     *
     * @param bookingDate The bookingDate field of the Contact to be returned
     * @return The first Contact with the specified bookingDate
     */
/*    Booking findByCustomerID(Long customerID) {
        ...
        // criteria.select(booking).where(cb.equal(booking.get(Contact_.bookingDate), bookingDate));
 ...
 /*   Booking findByFlightID(Long flightID) {
        ...
        Root<Booking> booking = criteria.from(Booking.class);
        // Swap criteria statements if you would like to try out type-safe criteria queries, a new feature in JPA 2.0.
        // criteria.select(booking).where(cb.equal(booking.get(Contact_.flightID), flightID));
    ...
    Booking create(Booking booking) throws ConstraintViolationException, ValidationException, Exception {
        log.info("BookingRepository.create() - Creating " + booking.getCustomerID() + " " + booking.getFlightID());
    ...
    Booking update(Booking booking) throws ConstraintViolationException, ValidationException, Exception {
        log.info("BookingRepository.update() - Updating " + booking.getCustomerID() + " " + booking.getFlightID());
    ...
    Booking delete(Booking booking) throws Exception {
        log.info("BookingRepository.delete() - Deleting " + booking.getCustomerID() + " " + booking.getFlightID());
