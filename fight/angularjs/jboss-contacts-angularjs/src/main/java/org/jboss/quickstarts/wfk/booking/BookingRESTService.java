...
public class BookingRESTService {
    ...
    /**
     * <p>Search for and return a Booking identified by customer address.<p/>
     *
     ...
     *
     *
     * @param customer The string parameter value provided as a Booking's customer
     * @return A Response containing a single Booking
     */
    @GET
    @Path("/customerID/{id:[0-9]+}")
    public Response retrieveBookingByCustomerID(@PathParam("id") long id) {
       // List<Book> book = service.findAllByCustomerId(id);
        Booking booking = service.findAllByCustomerID(id);

        if (booking == null) {
            throw new WebApplicationException(Response.Status.NOT_FOUND);
        }
        log.info("findById " + id + ": found Contact = " + booking.getCustomerID() + " " + booking.getFlightID() + " " + booking.getBookingDate() + " " 
            + booking.getId());
    ...
    public Response retrieveBookingByFlightID(@PathParam("id") long id) {
       // List<Book> book = service.findAllByFlightId(id);
        Booking booking = service.findAllByFlightID(id);
        if (booking == null) {
            throw new WebApplicationException(Response.Status.NOT_FOUND);
        }
        log.info("findById " + id + ": found Contact = " + booking.getCustomerID() + " " + booking.getFlightID() + " " + booking.getBookingDate() + " " 
    ...
    public Response retrieveBookingById(@PathParam("id") long id) {
        ...
        log.info("findById " + id + ": found Booking = " + booking.getFlightID() + " " + booking.getBookingDate() + " " + booking.getCustomerID() + " " + booking.getId());
    ...
    public Response createBooking(Booking booking) {
        log.info("createBooking started. Booking = " + booking.getBookingDate() + " " + booking.getFlightID() + " " + booking.getCustomerID() + " "  + booking.getId());
        if (booking == null) {
        ...
        try {
            ...
            log.info("createBooking completed. Booking = " + booking.getBookingDate() + " " + booking.getFlightID() + " " + booking.getCustomerID() + " " + booking.getId());
            ...
            //responseObj.put("bookingDate", "That bookingDate is already used, please use a unique bookingDate");
            ...
            if(e.toString().contains("date"))
               responseObj.put("bookingDate", "That bookingDate is  existed, Please check you information carefully");
               ...
               responseObj.put("flightID", "That flightID is not existed, Please check you information carefully"); 
                 if(e.toString().contains("booking"))
                     responseObj.put("booking", "That flightID and/or date are existed, Please check you information carefully");
    ...
    public Response updateBooking(@PathParam("id") long id, Booking booking) {
        ...
        log.info("updateBooking started. Booking = " + booking.getBookingDate() + " " + booking.getFlightID() + " " + booking.getCustomerID() + " " + booking.getId());
        ...
        try {
            ...
            log.info("updateBooking completed. Booking = " + booking.getBookingDate() + " " + booking.getFlightID() + " " + booking.getCustomerID() + " " + booking.getId());
        } catch (ConstraintViolationException ce) {
            log.info("ConstraintViolationException - " + ce.toString());
            // Handle bean validation issues
            builder = createViolationResponse(ce.getConstraintViolations());
    ...
    public Response deleteBooking(@PathParam("id") Long id) {
        ...
        try {
            ...
            log.info("deleteBooking completed. Booking = " + booking.getBookingDate() + " " + booking.getFlightID() + " " + booking.getCustomerID() + " " + booking.getId());
        } catch (Exception e) {
            log.info("Exception - " + e.toString());
            // Handle generic exceptions
            Map<String, String> responseObj = new HashMap<String, String>();
